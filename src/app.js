...
function createApp({ env = process.env } = {}) {
  ...
  if (isSwaggerEnabled(env)) {
    ...
    app.get("/docs", swaggerUi.setup(openapi, { customSiteTitle: "Inchill Adapter API" }));
    app.use("/docs", swaggerUi.serve);
  }
  app.use("/api/auth", internalAuth, authRoutes);
  app.use("/api/bot", internalAuth, botRoutes);
  app.use("/api/v2", (req, res) => res.status(410).json({
    status: "ERROR",
    code: "V1_ONLY_API",
    message: "The Inchill API is V1-only. V2 client routes are disabled.",
  }));
