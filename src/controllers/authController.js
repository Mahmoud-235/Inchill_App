const { sendOtpApi, verifySmsAuthApi } = require("../services/inchillService");
const User = require("../models/User");
const { encryptSessionForStorage } = require("../integrations/inchill/session");
...
exports.sendOtp = async (req, res) => {
  ...
  if (!validPhone(phone) || !/^\d{1,4}$/.test(countryCode))
    return res.status(400).json({
      status: "ERROR",
      message: "A valid phone number and countryCode are required.",
    });
  const result = await sendOtpApi(phone, countryCode);
  if (!result.ok)
    return res
      .status(result.kind === "BLOCKED" ? 503 : 400)
      .json({ status: "ERROR", message: result.message, code: result.kind });
...
exports.verifyOtp = async (req, res) => {
    ...
    return res
      .status(
        [
          "BLOCKED",
          "MISSING_DEVICE_ID",
          "SESSION_ESTABLISHMENT_UNPROVEN",
  ...
  await User.findOneAndUpdate(
    { phone },
    {
      hagoUid: result.session.hagoUid,
      ...(result.session.hOpenId ? { hOpenId: result.session.hOpenId } : {}),
      hagoSession: encryptSessionForStorage(result.session.cookies),
      ...(country ? { hagoCountry: country } : {}),
      ...(language ? { hagoLanguage: language } : {}),
