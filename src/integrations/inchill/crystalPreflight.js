...
function validateCrystalTransferPreflight({
  amount,
  wallet,
  permissions,
  agency,
  ...
  if (!/^[1-9][0-9]{0,6}$/.test(amountText)) {
    return {
      ...
      message:
        "A positive whole-number Crystal amount of up to seven digits is required.",
    };
  }
  ...
  if (permission?.present && permission.status === 1) {
    return {
      ok: false,
      kind: "CRYSTAL_TRANSFER_PERMISSION_REQUIRED",
      message: "Crystal transfer is not available for this account.",
    };
  }
  ...
  if (balance === null) {
    return {
      ok: false,
      kind: "CRYSTAL_BALANCE_UNAVAILABLE",
      message:
        "A current Crystal balance is required before a controlled transfer.",
    };
  }
  if (transferAmount > balance) {
    return {
      ok: false,
      kind: "CRYSTAL_INSUFFICIENT_BALANCE",
      message: "The requested Crystal amount exceeds the available balance.",
    };
  }

  const transferMax = Number(agency?.transferMax);
  if (
    Number.isFinite(transferMax) &&
    transferMax > 0 &&
    transferAmount > transferMax
  ) {
    return {
      ok: false,
      kind: "CRYSTAL_TRANSFER_MAX",
      message: "The requested Crystal amount exceeds the transfer maximum.",
    };
  }
