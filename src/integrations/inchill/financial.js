const {
  buildTransferAccountRequest,
  parseTransferAccountResponse,
} = require("./mutation");
const {
...
function isControlledMutationSenderEnabled({
  ...
  try {
    ...
    return (
      ...
      config.controlledMutationMode &&
      Number.isFinite(config.maxAmount) &&
      config.maxAmount > 0
    );
  } catch {
    return false;
...
function buildSeqId(currentAuthenticatedUid, targetUid, unixMillis) {
  if (
    ...
    !targetUid ||
    !Number.isSafeInteger(Number(unixMillis))
  )
    throw new TypeError("valid transfer sequence inputs are required");
...
function createFinancialMutationClient({
  http,
  ymicro,
  turnover,
  ...
  if (!http || !ymicro || !turnover)
    throw new TypeError("financial mutation dependencies are required");
  ...
  return {
    async prepareTransfer(session, { targetId, amount, serviceType }) {
      ...
      try {
        ...
        return {
          ok: false,
          kind: "INVALID_TRANSFER",
          message: "The controlled transfer request could not be constructed.",
        };
      }
    },
    ...
    async sendPreparedTransfer(session, request, guard) {
      ...
      if (!isControlledMutationSenderEnabled(guard))
        return { outcome: "BLOCKED", upstreamCode: null, attempted: false };
      const headers = buildTurnoverHeaders(session);
      if (!headers)
      ...
      try {
        const response = await http.post(
          `${baseUrl}/agencypay/transfer_account`,
          form,
          { headers },
        );
    ...
    async reconcileReadOnly(session, historyQuery = {}) {
      const [wallet, history] = await Promise.all([
        ...
        turnover.getHistory(session, historyQuery),
      ]);
...
module.exports = {
  buildSeqId,
  selectDiamondTransferCurrency,
  transferOutcomeFromError,
  isControlledMutationSenderEnabled,
  createFinancialMutationClient,
};
