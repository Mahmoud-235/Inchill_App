...
function buildTransferAccountRequest({
  targetUid,
  transferAmount,
  currencyType,
  useGoldCurrency,
  ...
  if (targetUid == null || String(targetUid).trim() === "")
    throw new TypeError("targetUid is required");
  if (!Number.isFinite(Number(transferAmount)) || Number(transferAmount) <= 0)
    throw new TypeError("transferAmount must be positive");
  if (!Number.isInteger(Number(currencyType)))
    throw new TypeError("currencyType is required");
  if (typeof useGoldCurrency !== "boolean")
    throw new TypeError("useGoldCurrency must be boolean");
  if (typeof seqId !== "string" || seqId.trim() === "")
    throw new TypeError("seqId is required");
  return {
    appId: HAGO_APP_ID,
    sign: "",
    data: JSON.stringify({
