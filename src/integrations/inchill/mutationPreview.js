...
function buildCrystalMutationPreview({
  amount,
  now = () => PREVIEW_TIMESTAMP,
} = {}) {
  const transferAmount = Number(amount);
...
function buildNobilityMutationPreview({
  nobilityType,
  buyType = BUY_TYPES.PURCHASE,
} = {}) {
  const nobleType = NOBLE_TYPE_BY_NAME[nobilityType] || Number(nobilityType);
  if (!Number.isInteger(nobleType) || nobleType < 1 || nobleType > 4)
    throw new TypeError("valid nobility type is required");
  if (![BUY_TYPES.PURCHASE, BUY_TYPES.RENEW].includes(Number(buyType)))
    throw new TypeError("valid Noble buy type is required");
  return {
...
module.exports = {
  ...
  buildCrystalMutationPreview,
  buildNobilityMutationPreview,
};
