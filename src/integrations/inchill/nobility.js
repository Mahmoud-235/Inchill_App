...
const NOBLE_TYPES = Object.freeze({
  NONE: 0,
  KNIGHT: 1,
  VISCOUNT: 2,
  EARL: 3,
...
const NOBLE_STATUSES = Object.freeze({
  NONE: 0,
  NOT_PURCHASED: 1,
  NORMAL: 2,
  DUE_SOON: 3,
...
const NOBLE_TYPE_BY_NAME = Object.freeze({
  Knight: 1,
  Viscount: 2,
  Earl: 3,
  Duke: 4,
...
function validNobleType(value) {
  ...
  return Number.isInteger(type) &&
    type >= NOBLE_TYPES.KNIGHT &&
    type <= NOBLE_TYPES.DUKE
    ? type
    : null;
}
...
function buildGetUserNobleRequest(targetUid) {
  if (targetUid === null || targetUid === undefined || targetUid === "")
    throw new TypeError("target UID is required");
...
function parseNoblePack(pack) {
  ...
  return {
    ...
    diamond: Number.isFinite(Number(pack.total_rebate_diamond))
      ? Number(pack.total_rebate_diamond)
      : null,
  };
}
...
function parseCurrentNoble(payload) {
  ...
  return {
    type: Number.isInteger(Number(info.noble_type))
      ? Number(info.noble_type)
      : null,
    name: typeof info.noble_name === "string" ? info.noble_name : null,
    status: Number.isInteger(Number(info.noble_status))
      ? Number(info.noble_status)
      : null,
    remainingSeconds: Number.isFinite(Number(info.remain_seconds))
      ? Number(info.remain_seconds)
      : null,
  };
}
...
function decideNoblePurchase({
  ...
  const currentType = Number.isInteger(Number(current.type))
    ? Number(current.type)
    : NOBLE_TYPES.NONE;
  const currentStatus = Number.isInteger(Number(current.status))
  ...
  if (selected < currentType) {
    return {
      ok: false,
      kind:
        currentStatus === NOBLE_STATUSES.RENEW_PERIOD
          ? "LOWER_NOBLE_RENEW_PERIOD"
          : "LOWER_NOBLE_ACTIVE",
    };
  }
  const buyType =
    selected === currentType ? BUY_TYPES.RENEW : BUY_TYPES.PURCHASE;
  const subscriptionWarning = hasOtherActiveSubscription(
    subscriptions,
    selected,
  );
  const changeWarning =
    (currentStatus === NOBLE_STATUSES.NORMAL ||
      currentStatus === NOBLE_STATUSES.DUE_SOON) &&
    selected !== currentType;
  return {
    ok: true,
    buyType,
    requiresConfirmation: subscriptionWarning || changeWarning,
    confirmationReason: subscriptionWarning
...
function selectNoblePurchase(config, selectedNobleType, buyType) {
  ...
  const selected = Array.isArray(config)
    ? config.find((entry) => entry.type === type)
    : null;
  const pack =
    buyType === BUY_TYPES.RENEW
      ? selected?.renewInfo
      : buyType === BUY_TYPES.PURCHASE
        ? selected?.buyInfo
        : null;
  ...
  return {
    type,
    buyType,
    turnoverPackId: pack.turnoverPackId,
    diamond: pack.diamond,
  };
}

// Logical payload only. This module has no HTTP sender.
function buildBuyNobleByAgencyPayload({
  nobleType,
  buyType,
  buyerUid,
  diamond,
  ...
  if (!type || ![BUY_TYPES.PURCHASE, BUY_TYPES.RENEW].includes(Number(buyType)))
    throw new TypeError("valid Noble type and buy type are required");
  if (buyerUid == null || buyerUid === "")
    throw new TypeError("buyer UID is required");
  if (diamond === null || diamond === undefined)
    throw new TypeError("config-derived Diamond amount is required");
  if (typeof appName !== "string" || appName.trim() === "")
    throw new TypeError("app name is required");
  return {
    noble_type: type,
...
function normalizeNobleResponse(payload) {
  ...
  if (code === 30500)
    return { outcome: "REJECTED", code, kind: "BssDiamondNotEnough" };
  if (code === 30201)
    return { outcome: "REJECTED", code, kind: "BssRenewTimesOverLimit" };
...
module.exports = {
  ...
  buildGetUserGPSubStatusRequest,
  parseNobleConfig,
  parseCurrentNoble,
