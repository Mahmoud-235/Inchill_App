...
const NOBLE_YMICRO_URL = "https://api.inchillapp.com/ymicro/sapi";
...
function buildNobleYmicroMetadata(
  session,
  method,
  now = Date.now(),
...
function withReadOnlyStage(result, stage, diagnostics = {}) {
  const upstreamHttpStatus = Number.isInteger(result?.status)
    ? result.status
    : null;
...
function normalizeNobleHttpError(error) {
  ...
  return {
    kind: "UPSTREAM_HTTP_FAILURE",
    message: "Hago returned an upstream HTTP failure",
...
function normalizeNobilityReadiness(config, current) {
  return {
    current: {
      ...
      remainingDays:
        current.remainingSeconds === null
          ? null
          : Math.ceil(current.remainingSeconds / 86400),
    },
    available: config.map((entry) => ({
      ...
      purchasePackAvailable:
        entry.buyInfo?.turnoverPackId == null ? false : true,
      renewPackAvailable:
        entry.renewInfo?.turnoverPackId == null ? false : true,
    })),
  };
}
  ...
  async function request(session, method, payload) {
    ...
    if (!cookie || !metadata) {
      return {
        ok: false,
        kind: "NO_SESSION",
        message: "An active per-account Hago session is required",
      };
    }
    try {
      ...
      if (isRejectedYmicroResponse(response.data)) {
        return {
          ok: false,
          kind: "REJECTED",
          message: "Hago rejected the Nobility read-only request",
        };
      }
  ...
  return {
    async listAllNobleConf(session) {
      const result = await request(
        session,
        `${NOBLE_RPC.service}.${NOBLE_RPC.listMethod}`,
        buildListAllNobleConfRequest(),
      );
        ...
        : withReadOnlyStage(
            {
              ...
              kind: "UNKNOWN",
              message:
                "Hago returned an invalid Nobility configuration response",
            },
            "LIST_NOBLE_CONFIG",
          );
    },
    async getUserNoble(session, targetUid) {
      ...
      const diagnostics = {
        uidWireType: "number",
        method: `${NOBLE_RPC.service}.${NOBLE_RPC.userMethod}`,
      };
      if (nobleUid === null) {
        return withReadOnlyStage(
          {
            ok: false,
            kind: "INVALID_REQUEST",
            message: "A valid Hago target is required",
          },
          "GET_USER_NOBLE",
        );
      }
      let payload;
      try {
        payload = buildGetUserNobleRequest(nobleUid);
      } catch {
        return withReadOnlyStage(
          {
            ok: false,
      ...
      const result = await request(
        session,
        `${NOBLE_RPC.service}.${NOBLE_RPC.userMethod}`,
        payload,
      );
      if (!result.ok)
        return withReadOnlyStage(result, "GET_USER_NOBLE", diagnostics);
        ...
        : withReadOnlyStage(
            {
              ok: false,
              kind: "UNKNOWN",
              message: "Hago returned an invalid Nobility state response",
            },
            "GET_USER_NOBLE",
            diagnostics,
          );
    ...
    async getReadiness(session, targetUid) {
      ...
      return {
        ok: true,
        nobility: normalizeNobilityReadiness(
