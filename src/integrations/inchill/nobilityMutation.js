...
function normalizeNobilityOutcome(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload))
    return { outcome: "UNKNOWN", upstreamCode: null };
  ...
  if (normalized.outcome === "REJECTED")
    return {
      outcome: "REJECTED",
      upstreamCode: normalized.code ?? null,
      knownError: normalized.kind ?? null,
    };
...
function createNobilityMutationClient({
  http,
  uaas,
  ymicro,
  nobility,
  ...
  if (!http || !uaas || !ymicro || !nobility || !turnover)
    throw new TypeError("Nobility mutation dependencies are required");
    ...
    async preparePurchase(session, { targetId, nobilityType }) {
      ...
      if (sessionStatus?.status !== "VALID")
        return {
          ...
          message:
            "A valid Hago session is required before a Nobility purchase.",
        };

      const target = await ymicro.getTargetByVid(session, targetId);
      const buyerUid = normalizeNobleUid(target?.user?.uid);
      if (!target?.ok || buyerUid === null)
        return {
          ok: false,
          kind: "INVALID_TARGET",
          message: "Hago did not provide a valid target UID.",
        };
      ...
      if (!configuration?.ok)
        return {
          ok: false,
          kind: configuration?.kind || "NOBILITY_CONFIG_UNAVAILABLE",
          message: "Current Nobility configuration is unavailable.",
        };
      if (!currentState?.ok)
        return {
          ok: false,
          kind: currentState?.kind || "NOBILITY_STATE_UNAVAILABLE",
          message: "Current Nobility state is unavailable.",
        };
      if (!walletResult?.ok)
        return {
          ok: false,
          kind: walletResult?.kind || "WALLET_UNAVAILABLE",
          message: "A current Diamond balance is required.",
        };

      const decision = decideNoblePurchase({
        selectedNobleType: nobilityType,
      ...
      if (!decision.ok)
        return {
          ok: false,
          kind: decision.kind,
          message: "The selected Nobility level is not eligible for purchase.",
        };
      ...
      if (decision.requiresConfirmation)
        return {
          ok: false,
          kind: "NOBILITY_CONFIRMATION_REQUIRED",
          message: "This Nobility change requires explicit confirmation.",
        };

      const selected = selectNoblePurchase(
      ...
      if (
        !selected ||
        !Number.isFinite(selected.diamond) ||
        selected.diamond <= 0 ||
        selected.turnoverPackId == null ||
        selected.turnoverPackId === ""
      ) {
        return {
          ok: false,
          kind: "NOBILITY_CONFIG_UNAVAILABLE",
          message: "The selected Nobility configuration is incomplete.",
        };
      }
      ...
      if (balance === null)
        return {
          ok: false,
          kind: "NOBILITY_DIAMOND_BALANCE_UNAVAILABLE",
          message: "A current legacy Diamond balance is required.",
        };
      if (selected.diamond > balance)
        return {
          ok: false,
          kind: "NOBILITY_INSUFFICIENT_DIAMOND",
          message:
            "The selected Nobility price exceeds the available legacy Diamond balance.",
        };
      try {
        return {
          ok: true,
        ...
        return {
          ok: false,
          kind: "NOBILITY_CONFIG_UNAVAILABLE",
          message: "The selected Nobility configuration could not be prepared.",
        };
      }
    },

    async sendPreparedPurchase(session, request, guard) {
      if (!isNobilitySenderEnabled(guard))
        return { outcome: "BLOCKED", attempted: false, upstreamCode: null };
      const cookie = buildCookieHeader(session);
      const metadata = buildNobleYmicroMetadata(
        session,
        `${NOBLE_RPC.service}.${NOBLE_RPC.purchaseMethod}`,
        now(),
        runtime,
      );
      if (!cookie || !metadata)
        return { outcome: "UNKNOWN", upstreamCode: null, timeout: false };
      try {
