...
function parseYmicroUser(payload) {
  const info =
    payload?.result?.errcode === 0 && Array.isArray(payload.infos)
      ? payload.infos[0]
      : null;
  if (!info || info.uid == null) return null;
  return {
    uid: String(info.uid),
    vid: info.vid == null ? undefined : String(info.vid),
    nick: info.nick,
    avatar: info.avatar,
...
function parseTurnoverWallet(payload) {
  ...
  if (
    ...
    !Array.isArray(jsonMsg.accountList)
  )
    return null;
  const balances = {
    hagoDiamond: null,
...
function parseTurnoverHistory(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload))
    return null;
  if (payload.jsonMsg === undefined)
    return Object.keys(payload).length ? payload : null;
  if (
    typeof payload.jsonMsg === "object" &&
    payload.jsonMsg &&
    !Array.isArray(payload.jsonMsg)
  )
    return payload.jsonMsg;
