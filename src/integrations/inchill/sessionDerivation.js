...
function encryptRawBase64(plaintext, keyMaterial) {
  ...
  try {
    const cipher = crypto.createCipheriv(
      TRIPLE_DES_CIPHER,
      firstTripleDesKeyBytes(keyMaterial),
      SESSION_IV,
    );
...
function decryptRawBase64(ciphertext, keyMaterial) {
  try {
    ...
    return Buffer.concat([
      decipher.update(decodeRawBase64(ciphertext)),
      decipher.final(),
    ]).toString("utf8");
  } catch {
    throw invalidSessionDerivation();
...
function parseSsession(sSession, smsCode) {
  ...
  if (
    ...
    typeof parsed.sSessionKey !== "string" ||
    parsed.sSessionKey.length === 0
  ) {
    throw invalidSessionDerivation();
  }
...
function combineUaasCookie(authCiphertext, sT) {
  if (
    typeof authCiphertext !== "string" ||
    authCiphertext.length === 0 ||
    typeof sT !== "string" ||
    sT.length === 0
  ) {
    throw invalidSessionDerivation();
...
function deriveBrowserSession(payload, { smsCode, timestamp } = {}) {
  if (
    ...
    typeof payload.s_t !== "string" ||
    payload.s_t.length === 0
  ) {
    throw invalidSessionDerivation();
  }
