...
function generateSmsAuthTs({ smsCode, timestamp }) {
  if (smsCode == null || timestamp == null)
    throw new TypeError("smsCode and timestamp are required");
  ...
  const otpDigestHex = crypto
    .createHash("sha256")
    .update(normalizedSmsCode, "utf8")
    .digest("hex");
  ...
  return Buffer.concat([
    cipher.update(normalizedTimestamp, "utf8"),
    cipher.final(),
  ]).toString("base64");
}

function createSmsAuthTsProvider({ deviceIdProvider } = {}) {
  if (!deviceIdProvider || typeof deviceIdProvider.getDeviceId !== "function")
    throw new TypeError("deviceIdProvider is required");
  return async ({ smsCode, otp, timestamp, deviceId, ...context }) => {
    const resolvedDeviceId = await deviceIdProvider.getDeviceId({
      ...context,
      deviceId,
    });
    if (!resolvedDeviceId)
      return {
        ...
        message:
          "A trusted FingerprintJS-compatible deviceId is required for SMS authentication.",
      };
    return {
      ok: true,
      ts: generateSmsAuthTs({ smsCode: smsCode ?? otp, timestamp }),
