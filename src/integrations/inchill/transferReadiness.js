...
function supportedDiamondCurrency(value) {
  ...
  return currencyType === HAGO_CURRENCIES.HAGO_DIAMOND ||
...
function effectiveTransferCurrency(agencyDiamondCurrency) {
  return (
    supportedDiamondCurrency(agencyDiamondCurrency) ||
    DEFAULT_DIAMOND_TRANSFER_CURRENCY
  );
}
...
function permissionEntry(entries, funcName) {
  const entry = Array.isArray(entries)
    ? entries.find((item) => item && item.funcName === funcName)
    : null;
  return entry
    ? { present: true, status: numericOrNull(entry.status) }
...
function parseAgencyReadiness(payload) {
  ...
  if (!data || typeof data !== "object" || Array.isArray(data))
    return { isAgency: null, transferMax: null, agencyDiamondCurrency: null };
  return {
...
function parsePermissionReadiness(payload) {
  const data = payload?.data;
  return {
    transferPermission: permissionEntry(
      data?.permissions,
...
function parseWalletReadiness(wallet) {
  ...
  return {
    walletDiamondLegacyAvailable:
      Number.isFinite(Number(balances.hagoDiamond)) &&
      Number(balances.hagoDiamond) > 0,
...
function createTransferReadinessClient({
  ...
  if (!uaas || !turnover)
    throw new TypeError("transfer readiness dependencies are required");
  ...
  async function postAgencyInfo(session) {
    ...
    const country = String(
      session.country || process.env.HAGO_COUNTRY || "",
    ).toUpperCase();
    const form = createSignedForm({ appId: HAGO_APP_ID, sign: "", data: "" });
    try {
      const response = await http.post(
        `${baseUrl}/agencypay/query_agency_info?uid=${encodeURIComponent(session.hagoUid)}&country=${encodeURIComponent(country)}`,
        form,
  ...
  return {
    async getTransferReadiness(session) {
      ...
      const [agency, permissions, hasTransactionPassword, wallet] =
        await Promise.all([
          postAgencyInfo(session),
          postSignedEmpty(
            session,
            "/agencypay/permissions",
            parsePermissionReadiness,
          ),
          postSignedEmpty(
            session,
            "/agencypay/has_psw",
            parsePasswordReadiness,
          ),
          turnover.getWallet(session),
        ]);
      if (agency) {
        Object.assign(readiness, agency);
        readiness.effectiveTransferCurrency = effectiveTransferCurrency(
          agency.agencyDiamondCurrency,
        );
      }
      if (permissions) Object.assign(readiness, permissions);
      if (hasTransactionPassword !== null)
        readiness.hasTransactionPassword = hasTransactionPassword;
      if (wallet?.ok)
        Object.assign(readiness, parseWalletReadiness(wallet.wallet));
...
module.exports = {
  ...
  parsePermissionReadiness,
  parsePasswordReadiness,
  supportedDiamondCurrency,
  effectiveTransferCurrency,
  parseWalletReadiness,
  createTransferReadinessClient,
};
