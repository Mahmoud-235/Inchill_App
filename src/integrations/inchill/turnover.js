...
const TURNOVER_BASE_URL =
  process.env.INCHILL_TURNOVER_BASE_URL || "https://db-turnover.inchillapp.com";
...
function buildHistoryData(query = {}) {
  if (!query || typeof query !== "object" || Array.isArray(query))
    throw new TypeError("history query must be an object");
  const allowed = [
    "qtype",
    "optTypes",
    "page",
  ...
  if (!Number.isInteger(Number(data.qtype)) || !Array.isArray(data.optTypes))
    throw new TypeError("invalid history query");
  if (!validIntegerArray(data.optTypes, MAX_HISTORY_FILTER_ITEMS))
    throw new TypeError("invalid optTypes");
  if (
    !Number.isInteger(Number(data.page)) ||
    Number(data.page) < 1 ||
    Number(data.page) > MAX_HISTORY_PAGE
  )
    throw new TypeError("invalid history page");
  if (
    !Number.isInteger(Number(data.pagesize)) ||
    Number(data.pagesize) < 1 ||
    Number(data.pagesize) > MAX_HISTORY_PAGE_SIZE
  )
    throw new TypeError("invalid history pagesize");
  if ((data.startTime === undefined) !== (data.endTime === undefined))
    throw new TypeError("startTime and endTime must be supplied together");
  if (
    data.startTime !== undefined &&
    (!validTimestamp(data.startTime) ||
      !validTimestamp(data.endTime) ||
      Number(data.endTime) < Number(data.startTime) ||
      Number(data.endTime) - Number(data.startTime) > MAX_HISTORY_RANGE_MS)
  )
    throw new TypeError("invalid history time range");
  if (data.vid !== undefined && !validPositiveInteger(data.vid))
    throw new TypeError("invalid history vid");
  if (
    data.currencyTypeList !== undefined &&
    !validIntegerArray(data.currencyTypeList, MAX_HISTORY_FILTER_ITEMS)
  )
    throw new TypeError("invalid currencyTypeList");
  ...
  if (data.currencyTypeList !== undefined)
    data.currencyTypeList = data.currencyTypeList.map(Number);
...
function validIntegerArray(values, maxLength) {
  return (
    ...
    values.every(validPositiveInteger)
  );
}
...
function createTurnoverClient({
  ...
  return {
    async getWallet(session) {
      ...
      if (!headers || !data)
        return {
          ok: false,
          kind: "NO_SESSION",
          message: "An active per-account Inchill session is required",
        };
      const signed = signer.signData(data);
      try {
        const response = await http.get(
          `${baseUrl}/query/${HAGO_APP_ID}/1005`,
        ...
        return wallet
          ...
          : {
              ...
              message: "Inchill returned an invalid wallet response",
            };
      } catch (error) {
    ...
    async getHistory(session, query = {}) {
      ...
      let data;
      try {
        data = buildHistoryData(query);
      } catch (error) {
        return { ok: false, kind: "INVALID_REQUEST", message: error.message };
      }
      const signed = signer.signData(data);
      const form = createSignedForm({
        appId: HAGO_APP_ID,
        sign: signed.sign,
        data: signed.data,
      });
      try {
        const response = await http.post(
          `${baseUrl}/agencypay/queryAccountHistory`,
          form,
          { headers },
        );
        ...
        return history
          ...
          : {
              ok: false,
              kind: "MALFORMED_RESPONSE",
              message: "Hago returned an invalid account history response",
            };
      } catch (error) {
        return { ok: false, ...normalizeHttpError(error) };
      }
    ...
    normalizeWallet(payload) {
      return parseTurnoverWallet(payload);
    },
  };
}

module.exports = {
  createTurnoverClient,
