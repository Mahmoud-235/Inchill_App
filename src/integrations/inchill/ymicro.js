...
function createYmicroClient({
  ...
  async function request(session, method, payload) {
    ...
    try {
      const response = await http.post(
        "https://api.inchillapp.com/ymicro/sapi",
        { sequence: Number(metadata.sequence), ...payload },
        {
      ...
      return user
        ...
        : {
            ok: false,
            kind: "BUSINESS_ERROR",
            message: "Hago returned no matching user",
          };
    } catch (error) {
  ...
  return {
    getTargetByVid(session, vid) {
      ...
      return normalizedVid
        ? request(session, "Uinfo.GetUinfoByVidVer", {
            vids: [{ vid: normalizedVid }],
          })
        : Promise.resolve({
            ok: false,
            kind: "INVALID_REQUEST",
            message: "A valid Hago VID is required",
          });
    },
    getUserByUid(session, uid) {
      const normalizedUid = normalizeYmicroId(uid);
      return normalizedUid
        ? request(session, "Uinfo.GetUinfoByVer", {
            uids: [{ uid: normalizedUid, ver: 0 }],
          })
        : Promise.resolve({
            ok: false,
            kind: "INVALID_REQUEST",
            message: "A valid Hago UID is required",
          });
    },
  };
}
