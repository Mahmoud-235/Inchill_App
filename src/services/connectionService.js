...
const {
  ClientDataCipher,
  digestProtectedLookup,
} = require("../security/clientDataSecrets");
const { encryptSessionForStorage } = require("../integrations/hago/session");

function makeConnectionValues({
  clientId,
  phone,
  countryCode,
  deviceId,
  ...
  if (!session?.cookies?.hagouid || !session?.cookies?.uaasCookie)
    throw new Error("Complete upstream session is required");
  ...
  return {
    ...
    upstreamAccountDigest: digestProtectedLookup(
      dataKey,
      "upstream-account",
      String(session.cookies.hagouid),
    ),
    hagoSession: encryptSessionForStorage({
      hagouid: session.cookies.hagouid,
      uaasCookie: session.cookies.uaasCookie,
    }),
...
async function getOwnedConnection({
  ...
  if (
    ...
    !/^con_[A-Za-z0-9_-]{22}$/.test(connectionId)
  )
    return null;
  const query = Connection.findOne({ clientId, connectionId });
  return includeSession
    ? query.select("+hagoSession +upstreamAccountDigest")
    : query;
}
